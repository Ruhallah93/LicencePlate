...
class Labeling:
    ...
    def neater(self):
        ...
        for i in range(itr):
            ...
            neater = NEATER(alpha=0.999999, h=1 , b = 2)
            print(np.shape(y_rotation))
    ...
    def geris(self):
        ...
        y_prediction_noise = []
        train_len = len(self.X)
        itr = int(len(self.X_no_label) / train_len)
        for i in range(itr):
            print(i + 1, "/", itr)
            ...
            geri = GERIS(alpha=0.9, h=4)

            x_prediction, prediction = geri.sample(x_rotation, y_rotation, x_n, y_n, data_produced=0, metric='euclidean')
            y_prediction_rotation = np.append(np.array(y_prediction_rotation), prediction)
            ...
            geri = GERIS(alpha=0.9, h=4)
            x_prediction, prediction = geri.sample(x_noise, y_noise, x_n, y_n, data_produced=0, metric='euclidean')
            y_prediction_noise = np.append(np.array(y_prediction_noise), prediction)
