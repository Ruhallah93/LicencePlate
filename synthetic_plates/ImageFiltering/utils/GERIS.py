...
class GERIS(OverSampling):
    ...
    def sample(self, X, y, X_test, y_test, data_produced, metric='minkowski'):
        ...
        def utilities(prob):
            ...
            return util_min, util_maj



        def evolution( c_correct , c_corrupt , prob, synthetic, alpha=self.alpha):
            """
            ...
            """
            util_min, util_maj = utilities(prob)
            c_corrupt = c_corrupt + util_maj
            c_correct = c_correct + util_min
            prob_new = prob.copy()

            R1 = np.power((1 + alpha), c_correct)
            R2 = np.power((1 + alpha), c_corrupt)
            ...
            return prob_new , c_correct , c_corrupt ,

        # executing the evolution

        for _ in range(self.h):
            prob , cumulative_correct , cumulative_corrupt = evolution(cumulative_correct , cumulative_corrupt , prob, synthetic)
