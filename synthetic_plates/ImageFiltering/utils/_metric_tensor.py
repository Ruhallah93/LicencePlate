...
def pairwise_distances_mahalanobis(X, Y=None, tensor=None):
    ...
    if tensor is None:
        ...
        #     tensor[i][i] = 1 / (2 * (X.shape[1] - 3))
        # tensor[-1, -1] = 1 / (2 * (X.shape[1] - 3))
        # tensor[-2, -2] = 1 / 6
        # tensor[-3, -3] = 1 / 6
        # tensor[-4, -4] = 1 / 6

        tensor[-1, -1] = 1
        tensor[-2, -2] = ((X.shape[1] - 3) / 3) ** 36
        tensor[-3, -3] = ((X.shape[1] - 3) / 3) ** 36
        tensor[-4, -4] = ((X.shape[1] - 3) / 3) ** 36
...
class ClosestNeighborsInClasses:
    ...
    def fit_transform(self, X, y):
        ...
        return np.vstack([X_maj[~label_maj], X_min[~label_min]]), np.hstack(
            [np.repeat(0, np.sum(label_maj), np.repeat(1, np.sum(label_min)))])
